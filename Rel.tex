...
\TrinaryInfC{\(
  \Gamma \vdash
  \case \, v_0 \, \{ x_0 \mapsto v_1 ; x_1 \mapsto v_2 \}
  \colon t_2 
\)}
\DisplayProof
...
\RightLabel{(\(+\)-STEP-1)}
\UnaryInfC{\(
  \case \, \inl_t(v_0) \, \{ x_0 \mapsto v_1 ; x_1 \mapsto v_2 \} \rightarrow [x_0 \update v_0] v_1
  \)}
\DisplayProof
...
\RightLabel{(\(+\)-STEP-2)}
\UnaryInfC{\(
  \case \, \inr_t(v_0) \, \{ x_0 \mapsto v_1 ; x_1 \mapsto v_2 \} \rightarrow [x_1 \update v_0] v_2
  \)}
\DisplayProof
...
\AxiomC{\( \Gamma \vdash p \colon t_0 + t_1 \)}
\RightLabel{(\(+\)-ELIM-2)}
\UnaryInfC{\( \Gamma \vdash \lft(p) \colon t_1 \)}
\DisplayProof
...
\begin{center}
  \begin{tikzcd}
    \top \arrow{dd}[']{\sigma_p} \arrow{rr}{\sigma_v} & {} \arrow{dd}{v \models p \, [\sigma_v , \sigma_p]} & \times_{x\colon t \in \Gamma_v} t \arrow{dd}{v} \\
    &                               &                      \\
