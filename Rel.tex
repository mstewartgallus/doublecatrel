...
{\taut
  \apply{V}
  \UnaryInf$ \bullet , \, X \colon t \fCenter X \colon t $
  \apply{\(\times\)I}
  \UnaryInf$ \bullet \fCenter  \lambda X \colon t . \, X \colon t \otimes t $
  \DisplayProof}
\hfill
{\taut
  \apply{V}
  \UnaryInf$ \Delta , \,  X \sat N \fCenter  x_t \sat N $
  \apply{\(\times\)I}
  \UnaryInf$ \Delta \fCenter \lambda X \colon t . \, X  \sat N , N $
  \DisplayProof}
...
\begin{center}
  ...
  \[ f \circ_t g = \lambda X \colon t . \, f \, (g \, X) \]
...
\begin{prooftree}
  ...
  \Axiom$ \bullet \fCenter f \sat N_1 , \, N_2 $

  \Axiom$ \bullet \fCenter g \sat N_0 , \, N_1 $

  \taut
  \apply{V}
  \UnaryInf$ \bullet , \, X \sat N_0 \fCenter X \sat N_0  $

  \apply{\(\times\)E}
  \BinaryInf$ \bullet , \, X \sat N_0 \fCenter g \, X \sat N_1 $

  \apply{\(\times\)E}
  \BinaryInf$ \bullet , \, X \sat N_0  \fCenter  f \, (g \, X)  \sat N_2 $
  \apply{\(\times\)I}
  \UnaryInf$ \bullet \fCenter \lambda X \colon t_0 . \, f \, (g \, X)  \sat N_0 , N_2 $
\end{prooftree}
...
\begin{center}
  ...
  $$ P(\bullet, \rho)_{\rho \colon \Gamma} = \bullet \vdash [\mathrel{:=} \rho] v \Downarrow N_\rho  $$

  \begin{tikzcd}
   \unit \arrow{dd}[']{\sigma} \arrow{rr}{P} & {} \arrow[Rightarrow]{dd}{\sigma \fCenter E \sat N_\rho} & \mathop{\bigotimes}\limits_{x\colon t \mathrel{\in} \Gamma} t \arrow{dd}{v} \\
    &                               &                      \\
    \mathop{\bigotimes}\limits_{X\colon t \mathrel{\in} \Delta} t \arrow{rr}[']{E}                           & {}                            & t                   
  \end{tikzcd}
\end{center}
...
\UnaryInf$\bullet \fCenter R \sat o, o $
\DisplayProof

\name{trans}
\Axiom$ \bullet \fCenter  \lambda (x\colon O). R \, (R \, x)  \sat o_0, o_1 $
\side{\bullet \fCenter o_1, o_0 \colon O \times O}
\UnaryInf$ \bullet \fCenter  R \sat o_0, o_1 $
\DisplayProof

\name{sym}
\Axiom$ \bullet \fCenter  R \sat o_0, o_1  $
\side{\bullet \fCenter o_1, o_0 \colon O \times O}
\UnaryInf$ \bullet \fCenter  R \sat o_1, o_0 $
\DisplayProof
...
{\name{\(+\text{I}_1\)}
\Axiom$\Delta \fCenter E_0 \sat N $
\UnaryInf$\Delta \fCenter E_0 ; E_1 \sat \inl_t(N) $
\DisplayProof
\hfill
\name{\(+\text{I}_2\)}
\Axiom$\Delta \fCenter  E_1 \sat N$
\UnaryInf$\Delta \fCenter  E_0 ; E_1 \sat \inr_t(N) $
\DisplayProof
}

\name{\(\mt\)I}
\Axiom$\Delta \fCenter E \sat \absurd_t(N) $
\UnaryInf$\Delta \fCenter \false \sat N $
\DisplayProof
...
& \textbf{Command Env} & \sigma & \bnfdef \ldots \mid \Delta , \, X \sat t 
\end{aligned}\]
...
\name{\(\Sigma\)I}
\Axiom$ \sigma , \,  X \sat t  \fCenter E \sat N $
\UnaryInf$ \sigma \fCenter  \Lambda X \colon \prop . E \sat \langle x \update t , N \rangle $
\DisplayProof
