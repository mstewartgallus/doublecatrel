...
\judge{VAR-S}
      {\langle v \mid x_t \rangle \in \sigma}
      {\langle v \mid x_t \rangle \with [\sigma]}

\bijudge{vR-S}
        {\(v_0 \step v_1\)}
        {\(\langle v_0 \mid p \rangle \with [\sigma]\)}
        {\(\langle v_1 \mid p \rangle \with [\sigma] \)}

\bijudge{pR-S}
        {\(p_0 \step p_1\)}
        {\(\langle v \mid p_0 \rangle \with [\sigma]\)}
        {\(\langle v \mid p_1 \rangle \with [\sigma] \)}

\axiom{\(\unit\)S}
      {\( \langle \coin \mid \success \rangle \with [\sigma] \)}

\judge{\(\times\)S}
      {\langle v_1 \mid p \rangle \with [ \sigma , \, \langle v_0 \mid x_t \rangle]}
      {\langle v_0 , v_1 \mid \forall (x \colon t ). p \rangle \with [\sigma]}
...
\begin{prooftree}
  ...
  \UnaryInfC{\( \langle v \mid x_t \rangle \with [ \sigma , \, \langle v \mid x_t \rangle] \)}
  \RightLabel{\scriptsize{(\(\times\)S)}}
  \UnaryInfC{\( \langle ( v , v ) \mid \forall (x \colon t). x_t \rangle \with [\sigma] \)}
...
\judge{\(+\text{S}_1\)}
      {\langle v \mid p_0 \rangle \with [\sigma]}
      {\langle \inl_t(v) \mid [ p_0 ; p_1 ] \rangle \with [\sigma]}
\hfill
\judge{\(+\text{S}_2\)}
      {\langle v \mid p_1 \rangle \with [\sigma]}
      {\langle \inr_t(v) \mid [ p_0 ; p_1 ] \rangle \with [\sigma]}
}

\judge{\(\mt\)S}
      {\langle \absurd_t(v) \mid p \rangle \with [\sigma]}
      {\langle v \mid \false_\Gamma \rangle \with [\sigma]}
...
\item[Substitutions] \hfill  \( \sigma \bnfdef \ldots \mid \sigma , \, \langle t \mid x \rangle \)
\end{description}
...
\begin{flushleft}
\judge{\(\Sigma\)S}
      {\langle v \mid p \rangle \with [ \sigma , \, \langle t \mid x \rangle]}
      {
  \langle \langle x \update t , v \rangle \mid \Forall (x \colon \prop). p \rangle \with [\sigma]
  }
\end{flushleft}
...
\begin{center}
  \begin{tikzcd}
    \unit \arrow{dd}[']{\sigma_p} \arrow{rr}{\sigma_v} & {} \arrow[Rightarrow]{dd}{\langle v \mid p \rangle \, [\sigma_v , \sigma_p]} & \times_{x\colon t \in \Gamma_v} t \arrow{dd}{v} \\
    &                               &                      \\
...
\judge{refl}
      ...
      {\langle o, o \mid R \rangle \with [\cdot]}

\bijudge{trans}
      {\( \cdot \vdash o_0, o_1 \colon O \times O \)}
      {\( \langle o_0, o_1 \mid \forall (x\colon O). R \, (R \, x) \rangle \with [\cdot] \)}
      {\( \langle o_0, o_1 \mid R \rangle \with [\cdot] \)}

\bijudge{sym}
      {\( \cdot \vdash o_1, o_0 \colon O \times O \)}
      {\( \langle o_0, o_1 \mid R \rangle \with [\cdot] \)}
      {\( \langle o_1, o_0 \mid R \rangle \with [\cdot] \)}
